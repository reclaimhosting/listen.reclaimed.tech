...
var roadshowAPI = api_url_div.textContent;
...
class Program {
  ...
  setTime() {
      ...
      if(navigator.userAgent.indexOf("Chrome") != -1){
        timeValue = `${this.date} ${this.time.replace(/\s/g, '\u202F')} America/New_York`;
        } else {
         timeValue =   `${this.date} ${this.time} America/New_York`;
        }
    this.programDateTime = DateTime.fromFormat(
       `${timeValue}`,
      'MM/dd/yyyy t z'
    );
    //console.log(this.programDateTime);
  }
  setComplete() {
    this.complete = true;
  }
